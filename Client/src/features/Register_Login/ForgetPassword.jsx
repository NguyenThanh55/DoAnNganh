...
const ForgetPassword = () => {
    ...
    const onSubmitSMS = async () => {
        ...
        if (ph === '' || ph.trim() === '' || username === '' || username.trim() === '') {
            ...
            try {
                const res = await axiosClient.post(endpoints['isUser'], formData);
...
//     const process = async () => {
//         let formData = new FormData();
//         // formData.append("email", taiKhoan.email);
//         formData.append("email", taiKhoan.phone);
//         setLoading(true);
//         let res = await authApi().post(endpoints['send-otp'], formData);
//         console.log(res.data.user);
//         console.log(res.data.otp);
//         if (res.data.user === "") {
//             setErr("");
//             setErr("Email của bạn không hợp lệ!!!");
//             console.log("Email của bạn không hợp lệ!!!");
//         } else {
//             setErr("");
//             setErr("Mã OTP đã được gửi đến địa chỉ email của bạn.");
//             setUser(res.data.user);
//             setOtp(res.data.otp);
//             console.log("Mã OTP đã được gửi đến địa chỉ email của bạn.");
...
//             'expired-callback': () => { }
//         });
//         console.log(">>>>>>Xủ lý xong hàm Captch");
//     }
// }
...
//     if (ph === '' || ph.trim() === '' || username === '' || username.trim() === '') {
//         setIsPhValid(false); // Nếu số điện thoại trống, đặt isPhValid thành false
//         setIsUserValid(false); // Nếu tên đăng nhập, đặt isUserValid thành false
//     } else {
//         let formData = new FormData();
//         formData.append("username", username);

//         const fetch = async (formatPh) => {
//             await signInWithPhoneNumber(auth, formatPh, appVerifier)
//                 .then((confirmationResult) => {
//                     window.confirmationResult = confirmationResult;
//                     setLoading(false);
